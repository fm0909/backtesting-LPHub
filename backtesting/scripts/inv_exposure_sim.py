...
class UniswapV3LPCalculator:
    ...
    def plot_r2_bar_chart_presentation_friendly(self, liquidity: float, save_filename: str = None):
        """
        Create a grouped bar chart showing price impact and impermanent loss vs inventory share for R=2 scenario
        Optimized for presentation slides (compact, clear, readable)
        Args:
            liquidity: The calculated liquidity value
            save_filename: Optional filename to save the plot
        ...
        # Custom sort: for 0% share, lowest IL first, then by ascending price
        def custom_sort(row):
            share = row['token0_share']
            price = row['price']
            il = abs(row['incremental_il'])
            if np.isclose(share, 0):
                # For 0% share, sort by lowest IL first, then by ascending price
                return (share, il, price)
            elif np.isclose(share, 100):
                # For 100% share, sort by descending price
                return (share, -price)
            else:
                return (share, price)
        data_sorted = sorted(data, key=custom_sort)
        token0_shares = [row['token0_share'] for row in data_sorted]
        price_impacts = [abs(row['price_impact']) for row in data_sorted]
        incremental_ils = [abs(row['incremental_il']) for row in data_sorted]
        
        x = np.arange(len(token0_shares))  # the label locations
        width = 0.38  # the width of the bars
        
        # Convert cm to inches: 16cm width, 7cm height
        fig_width_inches = 16 / 2.54  # 16cm to inches
        fig_height_inches = 7 / 2.54   # 7cm to inches
        
        fig, ax = plt.subplots(figsize=(fig_width_inches, fig_height_inches))
        
        bars1 = ax.bar(x - width/2, price_impacts, width, label='Price Impact (%)', color='blue', alpha=0.7)
        bars2 = ax.bar(x + width/2, incremental_ils, width, label='Impermanent Loss (%)', color='red', alpha=0.7)
        
        # Set x-ticks to inventory share values (show fewer ticks for clarity)
        tick_spacing = max(1, len(token0_shares)//10)
        ax.set_xticks(x[::tick_spacing])
        ax.set_xticklabels([f"{token0_shares[i]:.1f}" for i in range(0, len(token0_shares), tick_spacing)], fontsize=10)
        ax.set_xlabel('Token 0 Share (%)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Percent (%)', fontsize=12, fontweight='bold')
        ax.grid(True, axis='y', alpha=0.3)
        ax.legend(fontsize=12)
        
        # Add vertical line for initial price token0 share
        initial_sqrt_price = math.sqrt(self.initial_price)
        initial_token0 = liquidity * (self.sqrt_upper - initial_sqrt_price) / (initial_sqrt_price * self.sqrt_upper)
        initial_token1 = liquidity * (initial_sqrt_price - self.sqrt_lower)
        initial_total_value = initial_token0 * self.initial_price + initial_token1
        initial_token0_share = (initial_token0 * self.initial_price / initial_total_value) * 100
        
        # Find the closest x index to the initial token0 share
        closest_idx = np.argmin(np.abs(np.array(token0_shares) - initial_token0_share))
        ax.axvline(x=closest_idx, color='black', linestyle='--', alpha=0.8, linewidth=2, label=f'Initial Position ({initial_token0_share:.1f}%)')
        ...
        if save_filename:
            ...
            full_path = os.path.join(output_dir, save_filename)
            plt.savefig(full_path, dpi=300, bbox_inches='tight')
            print(f"R=2 grouped bar chart saved to {full_path}")
            plt.close()
        else:
            plt.show()
...
def main():
    ...
    # Create R=2 bar chart for price impact and impermanent loss
    print("\n" + "=" * 80)
    print("CREATING R=2 BAR CHART FOR PRICE IMPACT AND IMPERMANENT LOSS")
    print("=" * 80)
    
    # Get the R=2 calculator (index 3 in the list)
    r2_calculator = calculators[3]  # R=2 is the 4th scenario (index 3)
    r2_liquidity = results[3]['liquidity']
    
    # Create the bar chart
    r2_calculator.plot_r2_bar_chart_presentation_friendly(
        r2_liquidity, 
        "r2_price_impact_il_bar_chart.png"
    )
