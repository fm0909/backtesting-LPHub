...
def plot_illiq_vs_volume_presentation(filtered_df, data_folder, pool_address):
    """
    Generates a presentation-friendly plot of ILLIQ vs Trade Volume, filtering out top 1% trade volume outliers.
    """
    if len(filtered_df) > 10000:
        plot_df = filtered_df.sample(10000, random_state=42)
    else:
    ...
    width_inches = 16 / 2.54  # 16 cm to inches
    height_inches = 7 / 2.54   # 7 cm to inches
    
    # Filter data to remove outliers for better visualization
    zoomed_df = plot_df[plot_df['ILLIQ'] < plot_df['ILLIQ'].quantile(0.95)]
    # Further filter out top 1% trade volumes
    upper_limit = zoomed_df['abs_tradevolume'].quantile(0.99)
    filtered_zoomed_df = zoomed_df[zoomed_df['abs_tradevolume'] < upper_limit]

    fig, ax = plt.subplots(figsize=(width_inches, height_inches))
    
    # Create scatter plot with ILLIQ on x-axis and trade volume on y-axis
    scatter = ax.scatter(filtered_zoomed_df['ILLIQ'], filtered_zoomed_df['abs_tradevolume'], 
                        c=filtered_zoomed_df['last_price_impact'], 
                        cmap='viridis', s=20, alpha=0.6, 
                        norm=plt.cm.colors.LogNorm())
    
    # Customize the plot
    ax.set_xlabel('ILLIQ', fontsize=12, fontweight='bold')
    ax.set_ylabel('Trade Volume', fontsize=12, fontweight='bold')
    
    # Add colorbar
    cbar = plt.colorbar(scatter, ax=ax, shrink=0.8)
    cbar.set_label('Price Impact (log scale)', fontsize=10, fontweight='bold')
    ...
    plt.tight_layout()
    save_path = os.path.join(data_folder, f'illiq_vs_volume_presentation_{pool_address}.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"Saved presentation-friendly ILLIQ vs Volume plot to {save_path}")
    plt.close()
...
def main():
    ...
    # Load the data - check if running from root or scripts directory
    ...
    plot_illiq_vs_volume_presentation(filtered_df, data_folder, pool_address)
